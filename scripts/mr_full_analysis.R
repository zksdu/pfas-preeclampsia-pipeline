# mr_full_analysis.R — 定量 MR v2：eQTLGen Z-score 经 MAF/N 重建 Beta/SE + 正规 coloc
...
## ---------- 2. 核心基因 eQTL 行（预抽取） + MAF 文件 ----------
eqtl <- fread(cmd = "gzip -cd data/mr_core_eqtl.gz", sep = "\t", quote = "", showProgress = TRUE)
...
af <- fread(cmd = "gzip -cd data/eqtlgen_snp_af.txt.gz", sep = "\t", quote = "",
            select = c("SNP", "hg19_chr", "hg19_pos", "AlleleA", "AlleleB", "AlleleB_all"),
            showProgress = TRUE)
setnames(af, "AlleleB_all", "AF_B")
eqtl <- merge(eqtl, af[, .(SNP, AF_B)], by = "SNP", suffixes = c("", ".af"))
cat("after AF merge:", nrow(eqtl), "\n")
# 去重（同基因同 SNP 保留 p 最小；同 chrpos 防 coloc 报重复）
...
mr_one_gene <- function(sym, fdr_th = 1e-5, gap = 1e5, max_iv = 30) {
  sub <- eqtl[GeneSymbol == sym & Pvalue < fdr_th & !is.na(beta_rec) & is.finite(se_rec)]
  ...
  list(ivs = nrow(m), beta = b, se = se, p = p, OR = exp(b), Q_p = q_p, method = method, detail = m)
}
res <- lapply(core, mr_one_gene); names(res) <- core
out <- rbindlist(lapply(core, function(g) {
  r <- res[[g]]
  data.table(gene = g, n_IV = r$ivs, method = if (!is.null(r$method)) r$method else NA,
...
fwrite(out, "results/mr/mr_full_summary.csv")
print(out)
...
if (nrow(loc_res)) { fwrite(loc_res, "results/mr/coloc_full_summary.csv"); print(loc_res) }
cat("MR FULL DONE\n")
