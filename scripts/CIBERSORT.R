...
#' @import utils
# base-R quantile normalization（替代 preprocessCore::normalize.quantiles，避免编译依赖）
quantile_normalize_base <- function(Y) {
  rn <- rownames(Y); cn <- colnames(Y)
  o <- apply(Y, 2, order)
  sorted <- apply(Y, 2, sort)
  ref <- rowMeans(sorted)
  out <- matrix(nrow = nrow(Y), ncol = ncol(Y))
  for (j in seq_len(ncol(Y))) out[o[, j], j] <- ref
